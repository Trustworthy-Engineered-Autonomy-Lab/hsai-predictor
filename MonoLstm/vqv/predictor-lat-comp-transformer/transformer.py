...
import lstm as evaluator


class Transformer(nn.Module):
    def __init__(
        self, embed_dim=32, num_heads=4, num_layers=4, mlp_hidden_dim=526, dropout=0
    ):
    ...
    def forward(self, x):
        ...
        output = self.transformer_encoder(x)  # (batch_size, 32, 32)
        return output
    ...
    def train_model(
        ...
        for epoch in range(epochs):
            ...
            if finLoss is None or finLoss > epoch_loss:
                finLoss = epoch_loss
                torch.save(
                    self.state_dict(), f"./weights/transformer_weights{horizon}.pth"
                )
                torch.save(self.state_dict(), "./weights/transformer_weights_pred.pth")
...
def eval_train_cc(
    ...
    images_folder="../safety_detection_labeled_data/",
    vae_weights="./weights/vae_weights_split.pth",
    transformer_weights="./weights/transformer_weights_pred.pth",
    seq_len=32,
    horizon=10,
    ...
    checkpoint = torch.load(
        "./weights/lstm_weights.pth", map_location=device, weights_only=True
    )
    ...
    if load_transformer_weights:
        checkpoint = torch.load(
            transformer_weights, map_location=device, weights_only=True
        )
        model.load_state_dict(checkpoint)
    model.eval()
    ...
    for h in range(horizon_init, horizon_limit + 1, horizon_increment):
        for l in lens:
            ...
            mse, acc = eval(
                ...
                seq_len=l,
                device=device,
                transformer_weights=f"./weights/transformer_weights{h}.pth",
            )
